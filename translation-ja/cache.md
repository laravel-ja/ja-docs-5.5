...
Laravelは多くのキャッシュシステムに読み書きしやすい、統一したAPIを提供します。キャッシュの設定は`config/cache.php`で設定します。アプリケーション全体でデフォルトとして使用するキャッシュドライバをこのファイルの中で指定します。[Memcached](https://memcached.org)や[Redis](http://redis.io)など、人気のあるキャッシュシステムをLaravelは最初からサポートしています。
